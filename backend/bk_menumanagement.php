...
function showMenus($isDeleted, $MotherMenID, $tab) {
    ...
    $allMenus = execsqlSRS($sql, "Search", []);
    ...
    foreach ($allMenus as $menu) {
        $menuById[$menu['MenID']] = $menu;
        $byMother[$menu['MotherMenID']][] = $menu;
    }

    $rendered = [];

    $buildRow = function($menu, $rowClass, $indent) {
        $menID   = $menu['MenID'];
        $checked = $menu['UnActive'] == 0 ? ' checked' : '';
        return "
        <tr class='{$rowClass}' data-id='{$menID}'>
            <td>" . htmlspecialchars($menID) . "</td>
            ...
            <td class='text-center'>
                <div class='custom-control custom-switch'>
                    <input type='checkbox' class='custom-control-input toggleMenuStatus'
                           id='menuStatus{$menID}' data-id='{$menID}'{$checked}>
                    <label class='custom-control-label' for='menuStatus{$menID}'></label>
                </div>
            </td>
            <td class='text-center'>
                <button class='btn btn-sm btn-info btnEditMenu mr-1' data-id='{$menID}'>
                    <i class='fas fa-edit'></i>
                </button>
                <button class='btn btn-sm btn-danger btnDeleteMenu' data-id='{$menID}'>
    ...
    $renderLevel = null;
    $renderLevel = function($motherID, $indentPrefix)
        use (&$renderLevel, &$byMother, &$rendered, $buildRow) {

        if (!isset($byMother[$motherID])) return "";
        $html = "";

        foreach ($byMother[$motherID] as $menu) {
            if (isset($rendered[$menu['MenID']])) continue;
            $rendered[$menu['MenID']] = true;

            $rowClass = ($menu['MotherMenID'] == 0) ? "table-primary font-weight-bold" : "";
            $indent = ($motherID != 0)
                ? $indentPrefix . "<i class='fa fa-arrow-right text-muted mr-1'></i>"
                : "";

            $html .= $buildRow($menu, $rowClass, $indent);
            $html .= $renderLevel($menu['MenID'], $indentPrefix . "&nbsp;&nbsp;&nbsp;&nbsp;");
        }
        return $html;
    };
    ...
    foreach ($byMother as $motherID => $children) {
        if ($motherID == 0) continue;
        if (isset($menuById[$motherID])) continue;
        foreach ($children as $menu) {
            if (isset($rendered[$menu['MenID']])) continue;
            $rendered[$menu['MenID']] = true;
            $html .= $buildRow($menu, "", "");
            $html .= $renderLevel($menu['MenID'], "&nbsp;&nbsp;&nbsp;&nbsp;");
        }
    }

    return $html ?: "<tr><td colspan='10' class='text-center text-muted'>No menus found</td></tr>";
}
...
function handleAddMenu() {
    ...
    try {
        ...
        if (!$lastId) {
            $result = execsqlSRS(
                "SELECT MenID FROM Sys_Menu WHERE Menucode = ? ORDER BY MenID DESC",
                "Search", [$code]
            );
            if (!empty($result) && isset($result[0]['MenID'])) {
                $lastId = $result[0]['MenID'];
        ...
        if ($menuID > 0) {
            ...
            echo json_encode(["success" => true, "message" => "Menu added successfully! (Refresh to see it)"]);
        }
...
function handleUpdateMenu() {
    ...
    $check = execsqlSRS(
        "SELECT COUNT(*) as count FROM Sys_Menu WHERE Menucode = ? AND MenID != ?",
        "Search", [$code, $menID]
    );
    if ($check[0]['count'] > 0) {
        echo json_encode(["success" => false, "message" => "DUPLICATE_CODE"]);
...
function handleSoftDelete() {
    ...
    execsqlSRS("UPDATE Sys_Menu SET IsDeleted = 1 WHERE MenID = ?", "Update", [$menID]);
...
function handleRestoreMenu() {
    ...
    execsqlSRS("UPDATE Sys_Menu SET IsDeleted = 0 WHERE MenID = ?", "Update", [$menID]);
    ...
    foreach ($UserMenus as $menuItem) {
        ...
        $html .= "<li class='{$liClass}' data-read='{$menuItem["MenID"]}'{$dataWidget}>
            <a href='#' class='nav-link'
               " . (!empty($menuItem["MenuLink"])
                    ? "id='callpages' data-pagename='{$menuItem["MenuLink"]}'"
                    : "id='clckdropdown' data-IDsubmenu='{$menuItem["MenID"]}'") . ">
                <i class='{$icon}'></i>
                <p>
                    {$menuItem["Menu"]}
...
switch ($request) {
    case "getAllMenus":
        echo showMenus(false, "0", "");
        exit;

    case "getDeletedMenus":
        echo showDeletedMenus();
        exit;
    ...
    case "getSingleMenuRow":
        echo showSingleMenuRow($_POST['menID'] ?? 0);
        exit;
