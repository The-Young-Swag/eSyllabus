...
function getSidebarMenu() {
    ...
    if (!$RID && isset($_POST['userRID'])) $RID = $_POST['userRID'];
    if (!$RID) return '';

    $html = '';

    // 1. Fetch all top-level menus for this role (MotherMenID = 0)
    // Only check the parent menu's Unactive/IsDeleted status
    ...
    foreach ($UserMenus as $menuItem) {
        ...
        // 2. Fetch children menus for this parent
